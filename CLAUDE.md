...
An invited user has an app-level `users` row (`username`, `email`, `role`) before they have any real Deploro credential. `POST /admin/invite` immediately calls Deploro's `email_password/signup` with a random throwaway password (never stored, never shown to anyone — `routes/admin.ts`'s `throwawayPassword()`), purely to trigger Deploro's own "Confirm your account" email — that's the actual invite notification, since Deploro has no custom "you've been invited" template and no admin-initiated "here's a link to set your password" primitive (verified live: `request-reset` silently no-ops until an `email_password` identity has already been confirmed once, so there's no way to skip straight to a working set-password link). `deploro_user_id` stays null (`pending` in the admin UI) until the invitee finishes setup and logs in for the first time — `POST /admin/users/:id/resend-invite` re-sends the same confirmation email by calling signup again (verified live that re-signup on an unconfirmed identity is safe, not an error).

Two emails are unavoidable given that constraint, so the flow is chained rather than left for the invitee to figure out: Deploro's Auth Site URL (`deploro auth site-url`) points at `/welcome` instead of the bare app root, so clicking "Confirm your account" lands on `Welcome.tsx` with `?verified=1`, which immediately asks for their email and fires `POST /auth/forgot-password` (works now that they're confirmed) — that reset email's link lands on `ResetPassword.tsx`, where they set the password they'll actually use. Don't use the admin-add endpoint (`deploro auth end-users add`/`POST /api/projects/:id/auth/users`) as the invite trigger instead — verified live that it only attaches a passwordless OTP identity, and OTP confirmation doesn't satisfy `request-reset`'s "confirmed email_password identity" requirement either, so it's strictly worse for this chain.
